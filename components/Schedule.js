...
const generateTimeComponents = () => {
  ...
  const genFn = (startTime, n, hStart, duration = 30) => {
    let res = [];
    for (let i = 0; i < n; i++) {
      res.push(<div key={`h${hStart + i}`}><TimePeriod tag={`${`${tagCount? tagCount : ' '}`}`} start={`${formatSec(startTime + i * duration)}`} end={`${formatSec(startTime + (i + 1) * duration)}`}/></div>);
      tagCount++;
    }
    return res
  }
  ...
  return (
    <>
      ...
      <GridLayout css={layoutCss} layout={layout} cols={12} rowHeight={42.25} width={1046} height={676} margin={[0,0]}>
        ...
        <div key="jumaat-empty"><FullColumn> </FullColumn></div>
